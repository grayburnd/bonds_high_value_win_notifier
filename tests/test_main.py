...
def get_empty_excel_bytes() -> bytes:
    ...
    data = [
        ...
        ["Skipped row 2"],
        ["column_header"],
    ]  ##Simulates two skipped rows and then header name, as first 2 rows skipped in main
...
def test_init_winnings_check_raises_with_missing_area():
    ...
    with pytest.raises(KeyError):
        init_winnings_check(
            absolute_path,
            "non-existent-area",
            winning_mock_env_vars["purchase_date"],
            winning_mock_env_vars["bond_value"],
        )
