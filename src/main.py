...
def main():
    area, purchase_date, bond_value = env_vars_init(
        os.getenv("PB_AREA"),
        os.getenv("PB_DATE_OF_PURCHASE"),
        os.getenv("PB_VAL_OF_BOND"),
    )
    ...
    results_path = check_if_winner(
        area_matches, path, area, purchase_date, bond_value
    )
    cleanup(path, results_path)


def env_vars_init(
    area_code: str | None, date_of_purchase: str | None, val_of_bond: str | None
    ...
    logger.info(
        f"""Resolved Value for required env vars is: PB_AREA: {pb_area}, PB_DATE_OF_PURCHASE: {pb_date_of_purchase}, PB_VAL_OF_BOND: {pb_val_of_bond}"""
    )
        ...
        try:
            ...
            if err.response is not None and err.response.status_code < 500:
                ...
                if err.response is not None:
                    logger.critical(
                        f"Failed with server error code {err.response.status_code}. Retrying in {wait:.2f}s"
                    )
                else:
                    logger.critical(
                        f"Got empty response from Server: {err.response}. Retrying in {wait:.2f}s"
                    )
                time.sleep(wait)
...
def create_data_frame(response: requests.Response) -> pd.DataFrame:
    ...
    data_frame: pd.DataFrame = pd.read_excel(  # type: ignore
        excel_data,
        skiprows=2,
    )  ##Skip first 2 as not headers
    if data_frame.empty:
...
def process_data_frame(data_frame: pd.DataFrame) -> pd.DataFrame:
    data_frame = data_frame.dropna(
        how="all", axis=1
    )  ##Drop all columns which are empty
    data_frame.columns = [
        str(col).lower().replace(" ", "_").strip() for col in data_frame.columns
    ]  ##Normalize and clean up column names.. remove white spaces, replace spaces with underscores and make lower case
    data_frame["area"] = [
        str(value).lower().replace("  ", " ").replace(" ", "_").strip()
        for value in data_frame["area"]
    ]  ##Normalize and clean up area names.. remove white spaces, replace spaces with underscores, replace double spaces with single spaces and make lower case
    ##TODO
    ##Vectorize the above
    ...
    expected_column_headers = [
        ...
        "val_of_bond",
        "dt_of_pur",
    ]  # Check if headers are in the trimmed down intended headers row, notably data_frame.columns
    missing_cols = [
        col for col in expected_column_headers if col not in data_frame.columns
    ]
    if missing_cols:
...
def convert_to_csv(data_frame: pd.DataFrame, path: Path) -> None:
    csv_text = data_frame.to_csv(
        index=False
    )  ##Convert to csv without indexed rows
    with open(path, mode="w", encoding="utf-8") as file:
        file.write(csv_text)


def init_winnings_check(
    path: Path,
    area: str | None,
    ...
    with open(path, mode="r", encoding="utf-8") as file:
        csv_dict_reader = csv.DictReader(
            file
        )  ##Convert the csv file into a python dictionary/map
        raw_area_codes: list[str] = [row["area"] for row in csv_dict_reader]

        unique_area_codes: set[str] = set(raw_area_codes)

        logger.info(
            f"Finding the closest matching area codes as per PB_AREA environment variable: {area}..."
        )

        area = (
            str(area).lower().replace("  ", " ").replace(" ", "_").strip()
        )  ##Normalize pb_area due to string alphabetic nature

        area_matches: list[str] = difflib.get_close_matches(
            area, unique_area_codes, n=3, cutoff=0.6
        )  # Returns 3 since London has 3 sections for example so will check date, amount for all of them
        logger.info(
            f"The closest matching area codes as per your PB_AREA environment variable are: {area_matches}"
        )
...
def check_if_winner(
    ...
    with open(path, mode="r", encoding="utf-8") as file:
        ...
        winning_matches_results: list[dict[str, str]] = [
            holding
            for holding in csv_dict_reader
            if holding["val_of_bond"] == bond_value
            and holding["area"] in area_matches
            and holding["dt_of_pur"] == purchase_date
        ]
        ...
        if winning_matches_results:
            ...
            logger.info(message)
            pretty_printed_result = json.dumps(
                winning_matches_results, indent=2
            )

            if (
                os.getenv("GITHUB_OUTPUT") is not None
            ):  ##GitHub stores the environment variable for us on GitHub runners
                with tempfile.NamedTemporaryFile(
                ...
                zip_file_name: str = (
                    "results.zip"  ##Used in GitHub actions workflow
                )
                ...
                with open(os.environ["GITHUB_OUTPUT"], "a") as file:
                    ...
                    file.write("CREATE_ARTIFACT=True\n")
                    return results_path

        else:
            message = "Unfortunately, there are no matches for you this month!"
            logger.info(message)
...
def cleanup(path: Path, results_path: Path | None):
    ...
    for temp_file in [path, results_path]:
        if temp_file is not None and temp_file.exists():
            logger.info(
                f"{temp_file} exists hence deleting..."
            )  ##Should be in logs instead
            temp_file.unlink()
            logger.info("Cleanup finished!")  ##Should be in logs instead
